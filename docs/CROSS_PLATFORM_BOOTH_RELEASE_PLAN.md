...
- [ ] 2: 機内モードで起動できたか
- [ ] 3: 緑「コピーしました」か、赤「手動でコピーする」欄が開いたか。赤なら、その文言と「すべて選択 → コピー」で TALTO に書式付きで貼れたか
- [ ] 4〜5: 見出し・太字が残ったか
- [ ] 気づいた表示崩れ・押しにくい箇所（スクリーンショットがあれば）
...
- [ ] タッチ主体の端末では「複数ファイルを追加」を前に出し、「フォルダを追加」は「PC 向け」と表示して後ろへ下げる（`maxTouchPoints > 1` かつ `(pointer: coarse)`。`webkitdirectory` の有無では判定しない）
- [ ] ソフトウェアキーボード表示中に下部の固定ボタンが入力欄を隠さないようにする（`visualViewport`）
- [ ] iPad 縦向き・狭い Split View は工程表示、横向き・広い幅は2カラム
- [ ] 読み込み中・変換中・コピー中の状態表示と、処理中の二重操作防止
- [ ] モバイル用の容量警告（初期案: スマホ 5MB/20MB、タブレット 10MB/30MB を警告基準。実機計測後に確定）
- [ ] 読み込み失敗時にファイル名と対処を表示
- [ ] OS の文字サイズ拡大への追従（固定 px を rem へ寄せる）

確認（Playwright と、可能な範囲で実機）:

- 代表幅 360 / 375 / 390 / 430 / 768 / 820 / 1024 で横スクロールなし（自動）
- 縦横回転後も工程と入力内容が保たれる
- 日本語入力中に固定ボタンが入力欄を覆わない（実機）
