...
3. 短い原稿（見出し・本文・太字）を貼って「確認」まで進み、「書式付きでコピー」を押す
...
- [x] `はじめにお読みください.txt`（`tools/templates/はじめにお読みください.txt` に改名。始め方・スマホ・流れ・困ったとき・注意・対応環境・改修者向けの順）
- [x] `README.md`（入手方法・使い方・対応環境・権利を前、開発者向けは末尾に分離）
- [x] 画面内の文章（開発者向けの語がないことを確認。ホーム・端末別案内・使い方ダイアログは Phase B までに整備済み）
- [x] `docs/MIGRATION_GUIDE.md`（起動方法を端末別に、スマホの古い記述を差し替え、手動コピーを追記。「解析根拠」は復元コードのパスを外し「エディタを観察した結果」に改めた）
...
→ 文章はそろった。制作者の確認と、可能なら第三者 1 人の試用を待つ。
